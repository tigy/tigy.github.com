...
(function () {
	...
	Demo.addEvent(window, 'load', function triggerReady(){
	 ...
	 */
	   ...
	   */
		...
		function prettyPrint(opt_whenDone) {
			...
			function doWork() {
				var endTime = (PR.useContinuation ?
						 clock['now']() + 250 /* ms */ :
						 Infinity);
...
Demo.initPage = function(project){
	...
	root = (std || document.createTextNode('').constructor) ? root.src : root.getAttribute('src', 5);
...
Demo.showPage = function () {
    ...
    var root = Demo.rootPath,
		url = location.href.replace(root, ''),
		module,
	...
	t.push(Dpl.configs.title);
	
	t.push('</h1><h2>');
	
	t.push(Dpl.configs.subtitle);
    ...
    for (var category in data) {
        ...
        if(!total) {
			...
			if(isLib)
	        	t[currenHeader] = '<h2>' + Demo.encodeHTML(category) + ' <small>(' + finished + '/' + total + ')</small></h2>';
    ...
    if(currentInfo) {
    	...
    	if(isLib) {
    		...
    		title.innerHTML = '<div class="demo-control-toolbar"><a href="javascript://切换显示页面上的全部显示源码的功能" class="demo" onclick="Demo.toggleViewSource()" id="demo-toggleviewsource">' + t[0] + '</a>&nbsp;&nbsp;&nbsp;<a href="javascript://切换所有查看源码按钮的状态" class="demo" onclick="Demo.toggleAllSource()" id="demo-togglesource">✦ 切换折叠</a>&nbsp;&nbsp;&nbsp;<a href="javascript://切换显示外围框架" class="demo" onclick="Demo.toggleView()" id="demo-toggleview">' + t[1] + '</a></div>';
    	}
        ...
        header.innerHTML = currentInfo[0] + '<small>' + currentInfo[1] + '</small>';
...
Demo.initPage('assets/data/project.js');