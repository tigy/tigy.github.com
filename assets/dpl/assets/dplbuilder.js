...
var DplBuilder = {
    ...
    updateDpls: function () {
        ...
        for (var i = 0; i < t.length; i++) {
            ...
            html += ' | <a class="demo-viewsource-toggle" href="javascript://删除对当前模块的引用" onclick="DplBuilder.deleteDpl(' + i + '); return false;">删除</a></span><a class="demo" target="_blank" href="' + Demo.getDemoUrl(dpl.path) + '">';

            if (/^-/.test(dpl.type)) {
                html += '<del>';
            }

            html += (DplBuilder.types[dpl.type] || '') + dpl.path;
    ...
    /**
	 * 将界面上的DplFile 对象应用到 UI信息。
	 */
    loadDplFile: function () {
        ...
        form.find('[name=path]').setText(t.path);
        form.find('[name=base]').setText(t.properties.base);
        form.find('[name=title]').setText(t.properties.title || '');
        form.find('[name=js]').setText(t.properties.js || '');
        form.find('[name=css]').setText(t.properties.css);
        form.find('[name=images]').setText(t.properties.images);
        form.find('[name=requires]').setText((t.requires || []).join('; '));
        form.find('[name=parseMacro]').setAttr('checked', t.properties.parseMacro);
        form.find('[name=defines]').setText(t.properties.defines);
        form.find('[name=removeTrace]').setAttr('checked', t.properties.removeTrace);
        form.find('[name=removeAssert]').setAttr('checked', t.properties.removeAssert);
        form.find('[name=removeConsole]').setAttr('checked', t.properties.removeConsole);
        form.find('[name=addHeader]').setAttr('checked', t.properties.addHeader);
        form.find('[name=resolveRefs]').setAttr('checked', t.properties.resolveRefs);
